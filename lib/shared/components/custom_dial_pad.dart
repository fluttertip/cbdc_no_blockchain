...
class CustomDialPad extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        padding: const EdgeInsets.all(10.0),
        // Use LayoutBuilder so we size buttons based on available space (prevents huge buttons on web)
        child: LayoutBuilder(
          builder: (context, constraints) {
            // compute a reasonable amount field width (max cap so it doesn't grow too large on web)
            final amountFieldWidth = min(constraints.maxWidth * 0.45, 260.0);

            // compute button width: divide available width by 3 and subtract gaps/margins
            final rawButtonWidth = (constraints.maxWidth - 48) / 3;
            final buttonWidth = rawButtonWidth.clamp(56.0, 120.0);
